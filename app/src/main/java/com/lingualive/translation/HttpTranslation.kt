...
internal object HttpTranslation {
    fun postJson(url: String, apiKey: String, body: JSONObject): JSONObject {
        ...
        } catch (e: TranslationException) { throw e }
        catch (e: Exception) { throw TranslationException("network error", e) }
    ...
    fun extractGoogleText(json: JSONObject): String {
        val rows = json.optJSONArray("data")?.optJSONArray("translations") ?: JSONArray()
